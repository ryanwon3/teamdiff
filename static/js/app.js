(function () {
  ...
  function renderLeaderMeta(data) {
    ...
    metaEl.innerHTML = `
      ...
      <ul class="gold-meta-stats">
        ...
        <li>Of those, with lane + Riot participant id: <strong>${escapeHtml(String(data.anchor_matches_with_lane_meta ?? "—"))}</strong></li>
        <li>Lane-opponent pair games: <strong>${escapeHtml(String(data.lane_games ?? "—"))}</strong></li>
        <li>With timeline gold ≤15 min: <strong>${escapeHtml(String(data.games_with_gold_at_15 ?? "—"))}</strong></li>
        <li>Min games per row: <strong>${escapeHtml(String(data.min_games ?? "—"))}</strong></li>
        <li>Avg Δ gold sort: <strong>${escapeHtml(data.lead_sort === "desc" ? "high → low" : "low → high")}</strong></li>
      </ul>
    `;
  }
  ...
  function updateLeaderboardSortUI(leadSort) {
    ...
    if (leadColHeader) {
      leadColHeader.textContent = desc
        ? "Avg Δ gold (opp − you) ≤15 min ↓"
        : "Avg Δ gold (opp − you) ≤15 min ↑";
    }
    if (sortBlurbEl) {
      sortBlurbEl.innerHTML = desc
        ? "Each row is a champion you’ve laned against. <strong>Avg Δ gold</strong> is their gold minus yours at the latest timeline sample ≤15&nbsp;min (per game), averaged. Sorted <strong>high→low</strong> (hardest average lanes first). Click a row for the curve."
        : "Each row is a champion you’ve laned against. <strong>Avg Δ gold</strong> is their gold minus yours at the latest timeline sample ≤15&nbsp;min (per game), averaged. Default sort is <strong>low→high</strong> (easiest average lanes first). Click a row for the curve.";
    }
  }
  ...
  function buildChart(payload) {
    ...
    const colorA = getComputedStyle(document.documentElement).getPropertyValue("--accent").trim() || "#c89b3c";
    const colorB = getComputedStyle(document.documentElement).getPropertyValue("--muted").trim() || "#8b9bb4";

    const dsA = {
      label: (anchor && anchor.name) || payload.champion_a_name || "Anchor",
    ...
    const xTitle = payload.mode === "level" ? "Avg champion level (bin)" : "Minute";
  ...
  async function fetchCurve() {
    ...
    try {
      ...
      setStatus("error", "Invalid curve JSON.");
      return;
    }
    if (!res.ok) {
    ...
    curveCaption.textContent =
      body.mode === "level"
        ? `Level mode: each point is mean gold over timeline frames in that level bin (samples per frame; games per point ${gMin}–${gMax}). Lane games: ${body.games_lane}, with overlapping timeline: ${body.games_curve}.`
        : `Time mode: minute 0–25; mean gold when both champions have a frame at that minute (games per point ${gMin}–${gMax}). Lane games: ${body.games_lane}, with overlapping timeline: ${body.games_curve}.`;
    buildChart(body);
  }
  ...
  async function loadGoldLeaderboard() {
    ...
    if (!leaders.length) {
      ...
      if (anyM === 0) {
        msg =
          "This champion does not appear in your DB for the current queue filter. Check MATCHUP_QUEUE_ID in .env (try removing it to use all queues), confirm collect_matches.py is running, and that seed accounts actually play this champ.";
      } else if (laneMeta === 0) {
        msg =
          `This champion appears in ${anyM} match(es), but rows lack lane / Riot participant id (common for games ingested before that was added). Re-collect into a fresh DB or delete old rows and re-run collect_matches.py with the current code.`;
      } else if (lg === 0) {
        msg =
          `You have ${laneMeta} match(es) with lane data for this champ, but no same-lane opponent pair (enemy lane must match). Often the opponent row is missing lane metadata—re-ingest those matches.`;
      } else if (g15 === 0) {
        msg =
          `Found ${lg} lane matchup game(s) but none have timeline rows usable for gold at ≤15 min. Run backfill_timelines.py for older matches, or keep collect_matches running.`;
      } else {
        msg =
          `Found ${g15} game(s) with gold@≤15 data, but no opponent reached min_games=${mg} after grouping. Lower “Minimum games vs each opponent” (try 0) or ingest more games.`;
      }
      setStatus("info", msg);
      return;
    }
